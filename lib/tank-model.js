...
export function buildFuelingEvent(input) {
  ...
  if (!tankSize || tankSize <= 0) throw new Error("Die Tankgröße muss größer als 0 sein.");
  if (!litersPurchased || litersPurchased <= 0) throw new Error("Die getankte Menge muss größer als 0 sein.");
  if (litersPurchased > tankSize) throw new Error("Die getankte Menge kann nicht größer als der Tank sein.");
  if (!fullTank && (missingLiters === null || missingLiters < 0 || missingLiters > tankSize)) {
  ...
  if (priceWasEntered && (pricePerLiter === null || pricePerLiter <= 0 || pricePerLiter > 10)) {
    throw new Error("Bitte einen gültigen Preis pro Liter eingeben, z. B. 1,699.");
  }
  ...
  return {
    ...
    timestamp,
    litersPurchased,
    pricePerLiter: pricePerLiter !== null && pricePerLiter > 0 ? pricePerLiter : null,
    totalCost: pricePerLiter !== null && pricePerLiter > 0 ? litersPurchased * pricePerLiter : null,
...
export function applyRangeConfirmation(profile, state, confirmation) {
  ...
  if (state?.asOf && new Date(confirmation.timestamp) < new Date(state.asOf)) return state;
  const tankSize = Math.max(1, finiteNumber(profile.tankSize) || 70);
...
export function rangeFreshness(state, currentLocation = null, options = {}) {
  if (!state?.asOf) return { fresh: false, reason: "missing", ageHours: null, movedKm: null };
  ...
  const sourceLocation = state.confirmation?.location || state.event?.location || null;
  const movedKm = distanceKm(sourceLocation, currentLocation);
  const moveThresholdKm = finiteNumber(options.moveThresholdKm) ?? 5;
  if (ageHours > maxAgeHours) return { fresh: false, reason: "age", ageHours, movedKm };
...
export function rangeEstimate(profile, state) {
  ...
  return { raw, usable: Math.max(0, raw * (1 - uncertainty) - reserveKm), level };
}
...
export function fullToFullConsumption(history = []) {
  ...
  const fullIndexes = chronological
    .map((event, index) => (event.fullTank && finiteNumber(event.odometerKm) !== null ? index : -1))
  ...
  if (interval.some((event) => finiteNumber(event.litersPurchased) === null)) return null;
  const liters = interval.reduce((sum, event) => sum + Number(event.litersPurchased), 0);
